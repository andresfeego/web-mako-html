...
<meta http-equiv="Content-type" content="text/html; charset=utf-8" />
<meta content='http://www.mako.guru/registro/imagenes/logo3_mako.png' property='og:image'/>
 ...
 mysql_query("SET NAMES 'UTF8'");?>
  ...
  <?php 
    ...
    if (empty($_POST['email1']) or empty($_POST['codigo'])) {
      header('Location: ../directorio/directorio3.php');
    }
...
<div class="bloque_principal">
 ...
 <div id="logo">
  ...
  <?php echo '
     ...
     <label class="title">Vendedor:<span class="required" aria-required="true">*</span></label> 
     <select class="element-input in2" name="vendedor">
        
        <option value="0">Sin vendedor</option>
';

  $sql="select * from  `vendedores` where activo=1  ORDER BY `vendedores`.`nombres` DESC ";
  $res2=mysql_query($sql);

  while($vendedor=mysql_fetch_array($res2)) {

    echo '<option value="'.$vendedor["identificacion"].'">'.$vendedor["nombres"].' '.$vendedor["apellidos"].'</option>';
  ...
  echo "</select>";
   ...
   ?>
     <?php echo '<input type="hidden" name="plan" value="'.$_POST['plan'].'"/>'; ?> 

 <div class="element-input in2"><label class="title">Responsable del registro:<span class="required" aria-required="true">*</span></label><input  maxlength="100"  placeholder=""  class="large" type="text" name="responsable"  aria-required="true">        <div id="errorRespon" ></div>
</div>

<div class="element-input in2"><label class="title">Acuerdo monetario:<span class="required" aria-required="true">*</span></label><input  maxlength="7"  placeholder=""  class="large" type="number" name="acuerdo"  aria-required="true">        <div id="errorAcuerd" ></div>
</div>
    </div>
  </div>
...
<div class="element-radio ">
    ...
    <div class="element-radio in3">
      ...
      <div class=" in3"id="costdomi"></div></td>
          ...
          <div id="map"></div>
          <div id="errorUbica" ></div>
          <div class="element-input in3" ><input class="large" type="hidden" name="lat"  aria-required="true" value="0"></div>
          <div class="element-input in3" ><input class="large" type="hidden" name="loug"  aria-required="true" value="0"></div>
          <div class="lblayuda ayudahorarios" style="width:150;" onclick="pedirUbicacion();" ><span class="span">Pedir mi Ubicacion</span></div>
...
<div id="errorCat" ></div>
 ...
 <a href="" name="palabras"></a>
    <div class="element-textarea in2"><label class="title">Palabras o frases clave, escríbelas separadas por comas:<span class="required" aria-required="true">*</span> <div id="chars">120</div></label><textarea id="text" class="small" name="palclave" cols="20" rows="5" maxlength="500"  placeholder="Separadas por comas"  aria-required="true" ></textarea></div>
...
<script>
  function validar(formulario){
    ...
    if (!formulario.responsable.value) {
       var divi=document.getElementById( "errorRespon");
      divi.innerHTML="<div class='errores' ><span class='span'>Escribe un responsable del registro para este establecimiento.</span></div>";
      erroresCometidos++;
    }else{
      var divi=document.getElementById( "errorRespon");
      divi.innerHTML="";
    ...
    if (!formulario.acuerdo.value) {
       var divi=document.getElementById( "errorAcuerd");
      divi.innerHTML="<div class='errores' ><span class='span'>Escribe un valor para acuerdo monetario.</span></div>";
      erroresCometidos++;
    }else{
      var divi=document.getElementById( "errorAcuerd");
      divi.innerHTML="";
...
<script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDh3jyWqBKAx7bhRlB4sJ8d0UYvGGm2Xdg&v=3&callback=initMap"></script>

<script>
  var map;
  function initMap(){
    map = new google.maps.Map(document.getElementById('map'),{center: {lat: 5.543743, lng: -73.357520},zoom: 18,mapTypeId: google.maps.MapTypeId.ROADMAP,styles: [{featureType: "poi", elementType: "labels", stylers: [ { visibility: "off" } ]}]});
  }

       var iconBase = 'https://maps.google.com/mapfiles/kml/shapes/';
        var icons = {
          makoLoc: {
            icon: 'imagenes/makoloc.png'
          }
        };
 ...
 function pedirPosicion(pos) {
   ...
   map.setZoom(18);
   var marker = new google.maps.Marker({position: centro, icon:icons['makoLoc'].icon, map: map, title:'Estas aqui !'});
 document.form1.lat.value=pos.coords.latitude;
 document.form1.loug.value=pos.coords.longitude;
...
function pedirUbicacion(){
  if (devi == 1) {
    var errorDiv = document.getElementById('errorUbica');
    errorDiv.innerHTML = "<div class='errores' ><span class='span'>Pedir ubicación solo funciona desde dispositivos móviles con gps, si deseas guardar tu ubicación abre la pagina principal desde tu móvil o tablet y continua el registro con el mismo correo electrónico y contraseña que usaste.</span></div>";
    document.form1.lat.value=1;
 document.form1.loug.value=1;
  } else{
