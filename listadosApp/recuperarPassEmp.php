...
$json = file_get_contents('php://input');
  ...
  $codigo = generarcodigo(8);
				...
				$sqlc="select c.*, e.* from codigos as c join empresa as e on e.codigo = c.codigo where email = '".$correo."'";
				$resc=mysql_query($sqlc);

  	if ($empresa=mysql_fetch_array($resc)) {
  		// email stuff (change data below)
$to = $correo; 
$from = "soporte@mako.guru"; 
$subject = "Contraseña temporal empresario - www.mako.guru"; 
  ...
  <div style='border: #f34437 solid 2px;margin: 1em; border-radius:  20px; height:  auto; float:  left; overflow:  hidden;'>
    ...
    <div style='width: 50%; float: left; height: 400px; vertical-align: middle; text-align: center; position: relative'>
      ...
      <div style='font-size:1.5em;color:rgb(46,54,56);top: 45%;position: relative;'>
        ...
        Ingresa este codigo : - ".$codigo." - como contraseña temporal para el ingrso al sistema de administracion de empresas MAKO.
      </div>
    </div>
  </div>
...
$body .= $message.$eol;
    ...
    $sql = "update codigos set passTemp='".$codigo."' , pass='".$codigo."'  where email='".$correo."'";
// send message
